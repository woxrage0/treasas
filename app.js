...
renderTemplate(res, req, "sponsor.ejs");
  ...
  app.get("/giriss", (req, res) => {
  
renderTemplate(res, req, "giriss.ejs");
});
...
client.channels.get('769865233815699456').send(`\`${req.user.username}#${req.user.discriminator}\` adlı kullanıcı \`${db.fetch(`botlar.${req.params.botID}.isim`)}\` adlı botu raporladı! \n**Sebep:** \`${ayar['mesaj-1']}\``)
}
if(ayar['mesaj-2']) {
...
client.channels.get('771257783759667211').send(embed)
}})
});
