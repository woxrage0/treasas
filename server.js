...
client.on("ready", () => {
  ...
  client.user.setActivity(`| ${db.get("linkler").length} Bot uptime ediliyor.`);
...
linkss = linkA
 sahipp = sahip
  ...
  app.post("/add", checkAuth, (req, res) => {
    let ayar = req.body;
  let link = ayar["link"];
    if (!ayar["link"]) return res.send("Link yazmalısın!");

 if(db.get("linkler").map(z => z.url).includes(link)) {
      return res.send("Zaten sistemde ekli!");
    } else {
      db.push("linkler", { url: link, owner: req.user.id });
      res.send("Sisteme eklendi " + req.user.id);
    }
  });
