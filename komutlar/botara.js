...
exports.run = async function (client, message, args) {
      ...
      if(!args[0]) {
      return message.channel.send('Lütfen botun ID giriniz..');
    }
  
     if (db.has('botlar')) {
      if (Object.keys(db.fetch('botlar')).includes(args[0]) === false) {
         return message.channel.send('Bot bulunamadı veya `/bot-bilgi <BOT ID>` şeklinde kullanın.');
   }
  }
